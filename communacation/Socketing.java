package com.win7.example.textplus.communacation;
...
{	//we must check the ip via cmd command line ipcinfig and take our new ip and try to connect to it
	private static final String AUTHENTICATION_SERVER_ADDRESS = "http://10.0.0.9/textplus/index.php";
...
//while we are listeninig to the socket while there is an open port that we can use and we can do thing and transfer data and get data with it
		while (listening) {
			try {//strat listingt to the port and recievie the connection from the server
	...
	///it wont get the connection from the server above in the while(listining)
	...
	{	///getting the hashmap sockets and shutting it down one by one via butting the vales in a iterator and doing shutfoing for all the sockets we saved at the hashma so we can do a well shutdoing for all the sockets that are connectedto the server
		for (Iterator<Socket> iterator = sockets.values().iterator();iterator.hasNext();)//for all the values in hashmap
		{
		...
		//clearing the hashmap from the sockets we saved
		sockets.clear();
		//making the listing varibel to false so we dont try to recise a connection
		this.stopListening();
	}
